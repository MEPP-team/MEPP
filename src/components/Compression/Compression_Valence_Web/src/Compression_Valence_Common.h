...
/* HEADER LENGTH.*/
const int HEADER_L_CLOSED = 1;
const int HEADER_L_COMPONENT_OPERATION = 8;
const int HEADER_L_QBIT = 6;
const int HEADER_L_NUM_VERTICES = 15;
const int HEADER_L_NUM_FACETS = 16;
const int HEADER_L_COLOR_QUANT = 8;
const int FREE = -1;
...
{		
	...
	{
		pVertex->Vertex_Flag_web = FREE;
		pVertex->Vertex_Number_web = i;
		pVertex->Vertex_Sign_web = NOSIGN;
	}

	// facets flag initialization.
	Facet_iterator pFace = NULL;
	for (pFace = pMesh.facets_begin(); pFace != pMesh.facets_end(); pFace++)
		pFace->Facet_Flag_web = FREE;		
}
...
int Find_Type(const Halfedge_handle &h, const int & valence)
{
	...
	if (valence == 3)
	{
		if ((h->vertex()->Vertex_Sign_web == MINUS) && (h->opposite()->vertex()->Vertex_Sign_web == PLUS))
			type = 1;
		else if ((h->vertex()->Vertex_Sign_web == PLUS) && (h->opposite()->vertex()->Vertex_Sign_web == MINUS))
			type = 2;
...
Color_Unit Get_Vertex_Color(const Halfedge_handle & h)
{
	...
	Resulting_color.c0 = h->vertex()->color_int_web(0);
	Resulting_color.c1 = h->vertex()->color_int_web(1);
	Resulting_color.c2 = h->vertex()->color_int_web(2);
...
{	
	Color_Unit Resulting_color;
	Resulting_color.c0 = v.color_int_web(0);
	Resulting_color.c1 = v.color_int_web(1);
	Resulting_color.c2 = v.color_int_web(2);
...
void Retriangulation(Polyhedron &pMesh, const Halfedge_handle & ch, const unsigned &valence, const unsigned & Vertex_number, const int & Component_ID)
{
	...
	if ((type == 1) || (type == 2) || (type == 4))
	{
		h->facet()->Facet_Flag_web = TO_BE_REMOVED;
		//h->facet()->Patch_Index = Vertex_number;
				
		h->facet()->normal() = Triangle_Normal(h);
		
		h->facet()->Component_Number_web = Component_ID;
	...
	else if ((type == 5) || (type == 8))
	{
		if (g->vertex()->Vertex_Sign_web == NOSIGN)
			g->vertex()->Vertex_Sign_web = PLUS;
		if (g->next()->vertex()->Vertex_Sign_web == NOSIGN)
			g->next()->vertex()->Vertex_Sign_web = MINUS;

		h = h->prev();		
		h = pMesh.split_facet(h, g); 
		
		h->facet()->Component_Number_web = Component_ID;
		h->opposite()->facet()->Component_Number_web = Component_ID;

		h->facet()->normal() = Triangle_Normal(h);
		h->opposite()->facet()->normal() = Triangle_Normal(h->opposite());
		
		h->facet()->Facet_Flag_web = TO_BE_REMOVED;		
		h->opposite()->facet()->Facet_Flag_web = TO_BE_REMOVED;
	...
	else if (( type == 6) || (type == 7))
	{
		if (g->vertex()->Vertex_Sign_web == NOSIGN)
			g->vertex()->Vertex_Sign_web = MINUS;
		if (g->next()->vertex()->Vertex_Sign_web == NOSIGN)
			g->next()->vertex()->Vertex_Sign_web = PLUS;
	...
	else if ((type == 9) || (type == 12))
	{
		...
		if (g->next()->vertex()->Vertex_Sign_web == NOSIGN)
			g->next()->vertex()->Vertex_Sign_web = MINUS;
		if (g->next()->next()->vertex()->Vertex_Sign_web == NOSIGN)
			g->next()->next()->vertex()->Vertex_Sign_web = PLUS;
	...
	else if (type == 11)
	{
		...
		if (g->next()->vertex()->Vertex_Sign_web == NOSIGN)
			g->next()->vertex()->Vertex_Sign_web = PLUS;
		if (g->next()->next()->vertex()->Vertex_Sign_web == NOSIGN)
			g->next()->next()->vertex()->Vertex_Sign_web = MINUS;
	...
	else if ((type == 13) || (type == 16))
	{
		...
		if (g->next()->next()->vertex()->Vertex_Sign_web ==NOSIGN)
			g->next()->next()->vertex()->Vertex_Sign_web = PLUS;
		if (g->next()->next()->next()->vertex()->Vertex_Sign_web ==NOSIGN)
			g->next()->next()->next()->vertex()->Vertex_Sign_web = MINUS;
	...
	else if ((type == 14) || (type == 15))
	{
		...
		if (g->next()->next()->vertex()->Vertex_Sign_web ==NOSIGN)
			g->next()->next()->vertex()->Vertex_Sign_web = MINUS;
		if (g->next()->next()->next()->vertex()->Vertex_Sign_web ==NOSIGN)
			g->next()->next()->next()->vertex()->Vertex_Sign_web = PLUS;
...
{	
   ...
   CGAL_For_all(vh_it, vh_it_end)
   {
	...
	/*if(Valence == 3)
	{
		int Ref = Border_edge->next()->vertex()->Vertex_Number_web;
		...
		CGAL_For_all(Hvc, Hvc_end)
		{
			if(Hvc->opposite()->vertex()->Vertex_Number_web == Ref)
				res = true;
		}
	}
	...
	else if(Valence == 4)
	{
		...
		{
			int Ref = Border_edge->opposite()->prev()->opposite()->next()->vertex()->Vertex_Number_web;
	...
	else
	{
		...
		if (valence == 4)
		{
			if ((type == 5) || (type == 8))
			{
				...
				CGAL_For_all(Hvc,Hvc_end)
				{
					if (Hvc->opposite()->vertex()->Vertex_Number_web == Points_index[1])
						check = true;
				}
			}
			...
			else if (( type == 6) || (type == 7))
			{
				...
				CGAL_For_all(Hvc,Hvc_end)
				{
					if (Hvc->opposite()->vertex()->Vertex_Number_web == Points_index[2])
						check = true;;
				}
		...
		if (valence == 5)
		{
			if ((type == 9) || (type == 12))
			{
				...
				CGAL_For_all(Hvc,Hvc_end)
				{
					if (Hvc->opposite()->vertex()->Vertex_Number_web == Points_index[3])
						check = true;
				}
			}
		...
		if (valence == 6)
		{
			...
			else if ((type == 14) || (type == 15))
			{
				...
				CGAL_For_all(Hvc,Hvc_end)
				{
					if (Hvc->opposite()->vertex()->Vertex_Number_web == Points_index[4])
						check = true;
				}
...
bool Remove_Edges(Polyhedron & pMesh, const Halfedge_handle & h, const int & type)
{
	...
	{
		//verification
		if (g->prev()->opposite()->facet()->Facet_Flag_web != FREE)
			check = true;
		if (check == false)
		{
			g = g->prev();
	...
	{
		//verification
		if (g->next()->opposite()->facet()->Facet_Flag_web != FREE)
			check = true;
		if (check == false)
		{
			g = g->next();
	...
	{
		g = g->prev()->opposite();
		if (g->facet()->Facet_Flag_web != FREE)
			check = true;
		g = g->next()->opposite();
		if (g->facet()->Facet_Flag_web != FREE)
			check = true;
	...
	else if ((type == 13) || (type == 16))
	{
		...
		if (check == false)
		{
			...
			if (g->next()->next()->next()->vertex()->Vertex_Sign_web == NOSIGN)
				g->next()->next()->next()->vertex()->Vertex_Sign_web = PLUS;
			if (g->next()->next()->next()->next()->vertex()->Vertex_Sign_web == NOSIGN)
				g->next()->next()->next()->next()->vertex()->Vertex_Sign_web = MINUS;
		}
	}
	...
	else if ((type == 14) || (type == 15))
	{
		...
		if (check == false)
		{
			...
			if (g->next()->next()->next()->vertex()->Vertex_Sign_web == NOSIGN)
				g->next()->next()->next()->vertex()->Vertex_Sign_web = MINUS;
			if (g->next()->next()->next()->next()->vertex()->Vertex_Sign_web == NOSIGN)
				g->next()->next()->next()->next()->vertex()->Vertex_Sign_web = PLUS;
		}
	}
