...
#include "mepp_component_Compression_Valence_Web_plugin.hxx"
...
typedef boost::shared_ptr<Compression_Valence_Web_Component> Compression_Valence_Web_ComponentPtr;

void mepp_component_Compression_Valence_Web_plugin::OnCompress()
{
	...
	if (mw->activeMdiChild() != 0)
	{
		...
		Compression_Valence_Web_ComponentPtr component_ptr = findOrCreateComponentForViewer<Compression_Valence_Web_ComponentPtr, Compression_Valence_Web_Component>(viewer, polyhedron_ptr);
		{
...
//cata
				bool Is_normal_flipping_selected = false; //dial.normal_flipping->isChecked();
...
////cata
//				bool Is_use_metric_selected = false; //dial.useMetric->isChecked();				
...
//				bool Is_bijection_selected = false; //dial.Use_Bijection->isChecked();
////end cata
				char File_Name[256];
...
{
	// here your code
	mepp_component_Compression_Valence_Web_plugin *mepp_component_plugin = NULL;
	for (int i=0; i<viewer->lplugin.size(); ++i) {
		if (dynamic_cast<mepp_component_Compression_Valence_Web_plugin*>(viewer->lplugin[i]) != 0) {
			mepp_component_plugin = dynamic_cast<mepp_component_Compression_Valence_Web_plugin*>(viewer->lplugin[i]);			
		}
	}

	if (mepp_component_plugin)
	{
		Compression_Valence_Web_ComponentPtr component_ptr = mepp_component_plugin->findOrCreateComponentForViewer<Compression_Valence_Web_ComponentPtr, Compression_Valence_Web_Component>(viewer, polyhedron_ptr);
...
void mepp_component_Compression_Valence_Web_plugin::load_P3DW_file()
{
	emit(mw->get_mainwindowActionOpen()->doSendParamsOpen(tr("Open P3DW File - from Valence"), tr("P3DW files (*.p3dw)"), Specific, mepp_component_plugin_interface::load_file_from_component));
}

void mepp_component_Compression_Valence_Web_plugin::OnMouseLeftUp(QMouseEvent *event)
{
	if (mw->activeMdiChild() != 0)
	{
		Viewer* viewer = (Viewer *)mw->activeMdiChild();
		...
		if (doesExistComponentForViewer<Compression_Valence_Web_ComponentPtr, Compression_Valence_Web_Component>(viewer, polyhedron_ptr)) // important !!!
		{
			Compression_Valence_Web_ComponentPtr component_ptr = findOrCreateComponentForViewer<Compression_Valence_Web_ComponentPtr, Compression_Valence_Web_Component>(viewer, polyhedron_ptr);
			...
			if (!component_ptr->IsDecompress)
			{				
				QMessageBox::information(mw, APPLICATION, tr("Decompression not possible: please load .p3dw file first."));
				return;
			}
...
void mepp_component_Compression_Valence_Web_plugin::OnMouseRightUp(QMouseEvent *event)
{
	if (mw->activeMdiChild() != 0)
	{
		Viewer* viewer = (Viewer *)mw->activeMdiChild();
...
void mepp_component_Compression_Valence_Web_plugin::OnMouseWheel(QWheelEvent *event)
{
	if (mw->activeMdiChild() != 0)
	{
		Viewer* viewer = (Viewer *)mw->activeMdiChild();
...
void mepp_component_Compression_Valence_Web_plugin::OnDecompress_all()
{
	Timer timer;
	timer.start();
...
void mepp_component_Compression_Valence_Web_plugin::OnDecompress_one_level()
{
...
void mepp_component_Compression_Valence_Web_plugin::OnDecompress_precedent_level()
{
...
void mepp_component_Compression_Valence_Web_plugin::OnDecompress_go_to_specific_level()
{
	// active viewer
	if (mw->activeMdiChild() != 0)
	{
...
void mepp_component_Compression_Valence_Web_plugin::OnDecompress_mesh_sequence_on_off()
{
	// active viewer
	if (mw->activeMdiChild() != 0)
	{
		...
		Compression_Valence_Web_ComponentPtr component_ptr = findOrCreateComponentForViewer<Compression_Valence_Web_ComponentPtr, Compression_Valence_Web_Component>(viewer, polyhedron_ptr);
		{
			if (!component_ptr->IsDecompress)
			{				
				QMessageBox::information(mw, APPLICATION, tr("Switch of decompression mode not possible: please load .p3dw file first."));
				return;
			}
...
void mepp_component_Compression_Valence_Web_plugin::ShowText(void)
{
	// active viewer
	if (mw->activeMdiChild() != 0)
	{
...
/*
void mepp_component_Compression_Valence_Web_plugin::OnJCW(void)
{
	// active viewer
	if (mw->activeMdiChild() != 0)
	{		
...
void mepp_component_Compression_Valence_Web_plugin::OnJCWdecompress(void)
{
	// active viewer
	if (mw->activeMdiChild() != 0)
	{
		...
		Compression_Valence_Web_ComponentPtr component_ptr = findOrCreateComponentForViewer<Compression_Valence_Web_ComponentPtr, Compression_Valence_Web_Component>(viewer, polyhedron_ptr);
		{
			...
			else
			{
				...
				{					
					...
					vector<PolyhedronPtr>::iterator it = viewer->getScenePtr()->get_begin_polyhedrons();	// MT
					viewer->getScenePtr()->insert_polyhedron(it + component_ptr->Process_level-1, New_mesh);
					viewer->getScenePtr()->set_current_polyhedron(component_ptr->Process_level);					
				}
			}
...
void mepp_component_Compression_Valence_Web_plugin::OnJCWdecompress_without_extraction(void)
{
	// active viewer
	if (mw->activeMdiChild() != 0)
	{
...
*/
Q_EXPORT_PLUGIN2(mepp_component_Compression_Valence_Web_plugin, mepp_component_Compression_Valence_Web_plugin);
