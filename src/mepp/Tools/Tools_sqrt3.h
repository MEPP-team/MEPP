...
class CSubdivider_sqrt3
{
  ...
  public:
    ...
    void subdivide_border(Polyhedron& P)
    {
      ...
      std::vector<Point3d> pts;  // store new smoothed points temporarily
      do {
          if ( e->opposite()->is_border())
      ...
      std::vector<Point3d>::iterator it = pts.begin();
      do {
          if ( e->opposite()->is_border()) {
              e->vertex()->point() = *it++;
              e->opposite()->vertex()->point() = *it++;
              e->opposite()->next()->vertex()->point() = *it++;
          } else if ( e->is_border()) {
              e->opposite()->vertex()->point() = *it++;
              e->vertex()->point() = *it++;
              e->next()->vertex()->point() = *it++;
          }
      } while ( e++ != last_e);
      CGAL_assertion( it == pts.end());
      CGAL_postcondition( P.is_valid());
    }

    // Create center vertex (modif MT)
    Halfedge_handle create_center_vertex(Polyhedron& P,
                              Facet_iterator f)
    {
    ...
    void smooth_border_vertices(Halfedge_handle e,
                                OutputIterator out)
    {
        CGAL_precondition( e->is_border());
