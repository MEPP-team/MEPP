...
#ifndef _POLYHEDRON_TOOLS_RULES_H_01292002
#define _POLYHEDRON_TOOLS_RULES_H_01292002
...
public:
  void edge_point_rule(Halfedge_handle edge, Point& pt) {
    ...
    Point p2 = edge->opposite()->vertex()->point();
    Point f1, f2;
    CatmullClark_rule::face_point_rule(edge->facet(), f1);
    CatmullClark_rule::face_point_rule(edge->opposite()->facet(), f2);
  ...
  void vertex_point_rule(Vertex_handle vertex, Point& pt) {
    ...
    for (int i = 0; i < n; i++, ++vcir) {
      ...
      R[1] += (S[1]+p2[1])/2;
      R[2] += (S[2]+p2[2])/2;
      CatmullClark_rule::face_point_rule(vcir->facet(), q);
      Q[0] += q[0];
      Q[1] += q[1];
      Q[2] += q[2];
    }
...
#endif //_POLYHEDRON_TOOLS_RULES_H_01292002
